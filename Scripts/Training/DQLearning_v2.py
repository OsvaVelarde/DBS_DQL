...
OUTPUT_PATH = '/mnt/BTE2b/DBS/2020/v2/Trainings/'
DATE = str(date.today())
...
if __name__ == "__main__":
    ...
    g12,g13,index_region = config_episode(g12_lims,g13_lims,num_episodes)
    ...
    for ep in range(num_episodes):
        ...
        for step in range(num_steps):
            ...
            # Compute Reward.
            dato1, dato2 ,reward = rew.reward_function(state, action[1], next_state, reward_cfg)
            ...
            if training_step > training_start:
                ...
                print('Step:',step,'Epsilon', '%.2f' % agent.epsilon,
                      'Action:',action[1],'LF =', '%.4f' % dato1, 'PLV =', '%.4f' % dato2, 'Reward =', '%.4f' % reward)
            ...
            if training_step%num_save == 0 and training_step > training_start:
                ...
                print(training_step,'%.2f' % agent.epsilon,'%.4f' % reward, '%.4f' % q_history, 
                      '%.4f' % q_evaluate, file = output_file, flush=True)    
        ...
        print(ep, index_region[ep], g12[ep], g13[ep], total_reward[0][0], file = reward_file)
